...
/*   Updated: 2022/06/10 20:52:11 by rabderus         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{ // сразу берет две вилки, не по очереди
	sem_wait(philo->base->forks);
	sem_wait(philo->base->print);
	printf("time:%lld phil_num:%d has taken a left fork\n",
		get_time() - philo->base->t_start, philo->id);
	sem_post(philo->base->print);
	
	sem_wait(philo->base->forks);
	sem_wait(philo->base->print);
	printf("time:%lld phil_num:%d has taken a right fork\n",
		get_time() - philo->base->t_start, philo->id);
	sem_post(philo->base->print);
